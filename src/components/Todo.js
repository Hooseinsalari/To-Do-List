...
const Todo = ({ todo, onDelete, onComplete, onEdit }) => {
  return (
    ...
    >
      <div
        className={todo.isComplete ? styles.todoLeftComplete : styles.todoLeft}
      >
        <button className={styles.checkBtn} onClick={() => onComplete(todo.id)}>
          <FaCheck className={styles.checkIcon} />
        </button>
        <p onClick={() => onComplete(todo.id)} className={styles.todoText}>
          {todo.text}
        </p>
      </div>
